...
public class DLPTextToBigQueryStreaming {
	 ...
	 * {@link DLPTextToBigQueryStreaming#run(TokenizePipelineOptions)} method to
	...
	public static PipelineResult run(TokenizePipelineOptions options) {
		...
		/*
		 * Steps: 1) Read from the text source continuously based on default interval
		 * e.g. 300 seconds - Setup a window for 30 secs to capture the list of files
		 * emited. - Group by file name as key and ReadableFile as a value. 2) Create a
		 * side input for the window containing list of headers par file. 3) Output each
		 * readable file for content processing. 4) Split file contents based on batch
		 * size for parallel processing. 5) Process each split as a DLP table content
		 * request to invoke API. 6) Convert DLP Table Rows to BQ Table Row. 7) Create
		 * dynamic table and insert successfully converted records into BQ.
		 */
		...
		PCollection<KV<String, Iterable<ReadableFile>>> csvFiles = p
				 ...
				 * seconds - Setup a window for 30 secs to capture the list of files emited. -
		...
		final PCollectionView<List<KV<String, List<String>>>> headerMap = csvFiles
				...
				.apply("Create Header Map",
						ParDo.of(new DoFn<KV<String, Iterable<ReadableFile>>, KV<String, List<String>>>() {
		...
		PCollection<KV<String, TableRow>> bqDataMap = csvFiles
				...
				.apply("File Handler",
						ParDo.of(new DoFn<KV<String, Iterable<ReadableFile>>, KV<String, ReadableFile>>() {
							@ProcessElement
							public void processElement(ProcessContext c) {
								String fileKey = c.element().getKey();
								c.element().getValue().forEach(file -> {
				...
				.apply("Process File Contents",
						ParDo.of(new CSVReader(NestedValueProvider.of(options.getBatchSize(), batchSize -> {
							if (batchSize != null) {
								return batchSize;
							} else {
								return DEFAULT_BATCH_SIZE;
							}
						}), headerMap)).withSideInputs(headerMap))

				// 5) Create a DLP Table content request and invoke DLP API for each processsing
				.apply("DLP-Tokenization",
						ParDo.of(new DLPTokenizationDoFn(options.getDlpProjectId(), options.getDeidentifyTemplateName(),
	 ...
	 * file contents in chunks and process it in non-monolithic fashion. For
	 * example: if a CSV file has 100 rows and batch size is set to 15, then initial
	 * restrictions for the SDF will be 1 to 7 and split restriction will be
	 * {{1-2},{2-3}..{7-8}} for parallel executions.
	 */
	static class CSVReader extends DoFn<KV<String, ReadableFile>, KV<String, Table>> {
		...
		public CSVReader(ValueProvider<Integer> batchSize, PCollectionView<List<KV<String, List<String>>>> headerMap) {
			lineCount = 1;
				...
				try (BufferedReader br = getReader(c.element().getValue())) {
					...
					if (csvHeaders != null) {
						...
						LOG.debug(
								...
								startOfLine, endOfLine, dlpTable.getRowsCount());
		...
		private List<String> getHeaders(List<KV<String, List<String>>> headerMap, String fileKey) {
			return headerMap.stream().filter(map -> map.getKey().equalsIgnoreCase(fileKey)).findFirst()
	 ...
	 * exist before this pipeline runs. As response from the API is received, this
	 * DoFn ouptputs KV of new table with table id as key.
	 */
	static class DLPTokenizationDoFn extends DoFn<KV<String, Table>, KV<String, Table>> {
		...
		public DLPTokenizationDoFn(ValueProvider<String> dlpProjectId, ValueProvider<String> deIdentifyTemplateName,
	...
	private static String getFileName(ReadableFile file) {
		...
		if (!fileKey[1].equals(ALLOWED_FILE_EXTENSION) || !fileKey[0].matches(TABLE_REGEXP)) {
			throw new RuntimeException("[Filename must contain a CSV extension "
					+ " BQ table name must contain only letters, numbers, or underscores [" + fileKey[1] + "], ["
					+ fileKey[0] + "]");
		}
